...
function output_bookmarks_recursive($bookmarks, $allow_edit, $deduplicate, $check_url, $cache_prefix, $update_status, $bookmark_json, $output = array('url' => '', 'folder' => '', 'urls' =>array(), 'preload' => ''), $level = '') {
  global $site_url, $cache_dir, $preview_filename_prefix, $sync_file_prefix;
...
<a class="url touchOver'.($allow_edit ? ' search'.(!$check_url && isset($entry['not_found']) && $entry['not_found'] ? ' not-found' : '').(isset($entry['hide_not_found']) && $entry['hide_not_found'] ? '' : ' show-not-found') : '').'" data-search="'.htmlentities(strtolower($entry['name'].' '.$entry['url'])).'" href="'.$entry['url'].'"'.($allow_edit ? ' id="'.$level.'_'.$entry['id'].'" data-id="'.$level.'_'.$entry['id'].'"' : '').' data-type="url" '.($allow_edit ? 'data-level="'.$level.'_" ' : '').'title="'.htmlentities($entry['url']).'">
'.(((isset($entry['meta']['preview']) && ($r = $entry['meta']['preview']) && ($preview_file = $cache_dir . $preview_filename_prefix . $entry['id'] . '-' . sha1($r)) && (!file_exists($preview_file) || filesize($preview_file))) || ((!$check_url || !isset($entry['meta']['preview'])) && in_array(strtolower(substr(($r = (!$allow_edit && isset($entry['original_url']) && $entry['original_url'] ? $entry['original_url'] : $entry['url'])), strrpos($r, '.')+1)), array('jpg', 'jpeg', 'png', 'gif')))) ? '<img class="preview" id="preview-'.$level.'_'.$entry['id'].'" src="index.php?action=preview&id='.($allow_edit ? '' : $sync_file_prefix).$entry['id'].'&url='.urlencode($r).'&site='.urlencode($entry['url']).'" onerror="this.style.display=\'none\';" />' : '').'
...
<div class="entry'.(!$allow_edit ? ' sync' : '').'"'.($allow_edit ? ' id="entry-'.$level.'_'.$entry['id'].'"' : '').' style="display:block;" data-id="'.$level.'_'.$entry['id'].'">
<h3 class="folder_title touchOver" style="display:block;" data-id="'.$level.'_'.$entry['id'].'">
<div class="target touchOver'.(!$allow_edit ? ' noedit' : '').'"'.($allow_edit ? ' id="target-'.$level.'_'.$entry['id'].'" data-id="'.$level.'_'.$entry['id'].'"' : '').'>
<span class="folder_title_name search touchOver"'.($allow_edit ? ' onclick="document.getElementById(\'search\').value=\'\';searchStrFunction();location.href=\'index.php#'.$level.'_'.$entry['id'].'\';" id="title-'.$level.'_'.$entry['id'].'" data-id="'.$level.'_'.$entry['id'].'"' : '').' data-search="'.htmlentities(strtolower($entry['name'])).'" data-type="folder">&raquo;&nbsp;'.$entry['name'].'</span>
...
<div'.($allow_edit ? ' class="folder-wrap" id="folder-wrap-'.$level.'_'.$entry['id'].'"' : '').' style="display:block;">'."\n";
        if ($allow_edit)
        ...
        $output['url'] .= '</div><span class="target touchOver'.(!$allow_edit ? ' noedit' : '').'"'.($allow_edit ? ' id="target-'.$level.'_'.$entry['id'].'_0" data-id="'.$level.'_'.$entry['id'].'_0"' : '').'>&nbsp;</span></div>'."\n";
      }
    }
  }
  return $output;

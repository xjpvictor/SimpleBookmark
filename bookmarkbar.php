...
echo 'bml.innerHTML=\'<div style="margin:0 10px !important;" id="spbkmk-wrap" ondragover="event.preventDefault();" ondrop="if(event.stopPropagation()){event.stopPropagation();}var str=event.dataTransfer.getData(\\\'text/html\\\');this.parentNode.innerHTML=\\\'\\\';var script=document.createElement(\\\'script\\\');script.src=\\\''.$site_url.'bookmarkbar.php?'.(isset($_GET['folder']) ? 'folder='.$_GET['folder'].'&' : '').'entry=\\\'+encodeURIComponent(str);bml.appendChild(script);">\'+bml.innerHTML+\'</div>\';';
